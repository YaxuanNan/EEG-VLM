...
class LengthGroupedSampler(Sampler):
    ...
    def __iter__(self):
        if self.group_by_modality:
            ...
            # Arrange according to the obtained indices
            # Construct the relative path for the train_high_level_features.npy file
            resnet_features_path = os.path.join(current_dir, '..', '..', 'sleep_data', 'train_high_level_features.npy')
            # Load the resnet features
            resnet_features = np.load(resnet_features_path)
            # Reorder the resnet features according to indices
            reordered_resnet_features = resnet_features[indices]
            # Save the reordered high_level features
            resnet_shuffled_features_path = os.path.join(current_dir, '..', '..', 'sleep_data', 'train_high_level_shuffled_features.npy')
            np.save(resnet_shuffled_features_path, reordered_resnet_features)
